...
void EncodeAndWriteMavlink() {     
  ...
  //                           uint32_t h_acc, uint32_t v_acc, uint32_t vel_acc, uint32_t hdg_acc)
  // GPS_RAW_INT #24
  ...
  if (GPS_5002_flag && velyaw_5005_flag) {  
    ...
    mavlink_msg_gps_raw_int_pack(ap_sysid, ap_compid,  &S2Gmsg,
        ap24_utime_usec, ap_fixtype, ap_lat24, ap_lon24, ap_amsl24, ap_eph, ap_epv, ap_vel, ap_cog, ap_sat_visible, 0, 0, 0, 0, 0 );
    len = mavlink_msg_to_send_buffer(GCSbuf, &S2Gmsg);
    Write_To_GCS(); 
...
//                               uint8_t severity, const char *text)
    mavlink_msg_statustext_pack(ap_sysid, ap_compid, &S2Gmsg, ap_severity, ap_text);
    len = mavlink_msg_to_send_buffer(GCSbuf, &S2Gmsg);
    Write_To_GCS();  
